...
教学点：
  ...
  4. 后端 import 放在 main() 函数体内按需触发，避免 `fincli weather` 触发 rag_backend
     模块级副作用（FAISS 加载、Embedding 模型目录检查等）
"""
...
def main():
    ...
    # 后端 import 放在这里按需触发：weather 链路只 import weather_backend,
    # 不会触发 rag_backend 模块级副作用（FAISS 加载、bge-m3 路径检查等）
    if args.cmd == "weather":
        from src.weather_backend import get_weather
        print(get_weather(args.city))
    elif args.cmd == "list-companies":
        from src.rag_backend import list_companies
        print(list_companies())
    elif args.cmd == "search":
        from src.rag_backend import search_annual_report
